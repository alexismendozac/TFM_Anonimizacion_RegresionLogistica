...
def entrenar_y_evaluar_modelo(X_train, X_test, y_train, y_test, random_state=42):
    """
    Entrena un modelo RL y evalúa su rendimiento.
    ...
    Args:
        ...
        #n_estimators (int): Número de árboles en el RL
        random_state (int): Semilla para reproducibilidad
    ...
    # Mostrar coeficientes de las variables (importancia para regresión logística)
    plt.figure(figsize=(12, 6))
    importancias = pd.Series(modelo.coef_[0], index=X_train.columns)
    importancias = importancias.sort_values(ascending=False)
    sns.barplot(x=importancias.index, y=importancias.values)
    plt.title('Coeficientes de las Variables (Regresión Logística)')
    plt.xticks(rotation=90)
    plt.tight_layout()
    plt.show()
